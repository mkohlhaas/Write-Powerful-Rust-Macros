...
fn handle_expression(expression: Expr, token: Option<Semi>) -> Stmt {
  match expression {
    Expr::If(mut if_expr) => {
      let new_statements: Vec<Stmt> = if_expr
        .then_branch
        .stmts
        .into_iter()
        .map(|s| match s {
          Stmt::Macro(ref expr_macro) => {
            ...
            // emit_error! is a macro provided by the proc-macro-error crate. It allows you to
            // report compiler errors from within procedural macros without immediately aborting
            // compilation, allowing the macro to continue and catch multiple issues.
            // https://docs.rs/proc-macro-error/latest/proc_macro_error/#macros
            ...
            if output.map(|v| v.is_empty()).unwrap_or(false) {
              ...
              extract_panic_content(expr_macro)
                .map(|t| {
                  quote! {
                      // replace panic with Result/Err; panic msg becomes error message
      ...
      Stmt::Expr(Expr::If(if_expr), token) // a statement can be an expression(!)
    }
    Expr::While(mut ex_while) => {
      let new_statements: Vec<Stmt> = ex_while
        .body
        .stmts
        .into_iter()
        .map(|s| match s {
          Stmt::Macro(ref expr_macro) => {
            ...
            if output.map(|v| v.is_empty()).unwrap_or(false) {
              ...
              extract_panic_content(expr_macro)
                .map(|t| {
                  quote! {
                      return Err(#t.to_string()); // replace panic with Result
                  }
                })
                .map(syn::parse2)
                .map(Result::unwrap)
...
pub fn panic_to_result(_attr: TokenStream, item: TokenStream) -> TokenStream {
  ...
  let new_statements: Vec<Stmt> = ast
    ...
    .into_iter()
    .map(|s| match s {
      Stmt::Expr(expr, semicolon) => handle_expression(expr, semicolon),
      _ => s,
    })
    .collect();
