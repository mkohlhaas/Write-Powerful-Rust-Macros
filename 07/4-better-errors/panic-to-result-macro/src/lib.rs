...
fn handle_expression(expression: Expr, token: Option<Semi>) -> Result<Stmt, syn::Error> {
  if let Expr::If(mut if_expr) = expression {
    let new_statements: Result<Vec<Stmt>, syn::Error> = if_expr
      .then_branch
      .stmts
      .into_iter()
      .map(|s| match s {
      ...
      .collect();
    if_expr.then_branch.stmts = new_statements?;
...
fn extract_panic_content(macro_stmt: &StmtMacro) -> Option<TokenStream2> {
  let does_panic = macro_stmt
    .mac
    .path
    .segments
    .iter()
    .any(|path_segment| path_segment.ident.to_string().eq("panic"));

  if does_panic {
    Some(macro_stmt.mac.tokens.clone())
  } else {
    None
  }
}
...
#[proc_macro_attribute]
pub fn panic_to_result(_attr: TokenStream, item: TokenStream) -> TokenStream {
  let mut fn_block: ItemFn = syn::parse(item).unwrap();

  let signature_output = signature_output_as_result(&fn_block);
  let statements_output: Result<Vec<Stmt>, syn::Error> = fn_block
    .block
    .stmts
    .into_iter()
    .map(|stmt| {
      if let Stmt::Expr(expr, semicolon) = stmt {
        handle_expression(expr, semicolon)
      } else {
        Ok(stmt)
      }
    })
  ...
  match (statements_output, signature_output) {
    (Ok(new), Ok(output)) => {
      fn_block.block.stmts = new;
      fn_block.sig.output = output;
    }
  ...
  let last_statement = fn_block.block.stmts.pop();
  fn_block
    .block
    .stmts
    .push(last_statement_as_result(last_statement));

  fn_block.to_token_stream().into()
}
